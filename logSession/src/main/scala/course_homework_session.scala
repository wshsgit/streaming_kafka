...
object course_homework_session {
  ...
  def main(args: Array[String]) {
    ...
    val mytb2 = spark.sql("" +
      "select t2.id maxindex,t2.uid uid,t2.cid cid,t2.qid maxqid,t2.rt rt,t2.lt lt,t2.il il,t1.minct minct,t1.maxct maxct from " +
      "mytb1 t1 inner join mytb t2 on (t1.row_num == t2.row_num) ")
    mytb2.show(200)
    //    存入数据库
    insertMySQL("session_course_homework", mytb2)
